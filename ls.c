...
ls(const char *path, const struct entry *ent, int listdir)
{
	...
	if (!listdir)
		output(ent);
	else if (S_ISDIR(ent->mode) || (S_ISLNK(ent->mode) && S_ISDIR(ent->tmode)))
	{
...
main(int argc, char *argv[])
{
	...
	default:
		for (i = ds = fs = 0, fents = dents = NULL; i < (size_t)argc; ++i)
		{
			...
			// after we make the directory, we add it to dents & fents, which
			// essentially is a heap of directory & file entries
